% This is the file to simulate opinion dynamics in time-varying social
% influence networks
...
function [opinion_history, W_series] = simulate(X, C, T, epsilon, delta, seed)
    ...
    % Simulation loop
    for t = 1:T
        W = create_init_W_matrix(X, epsilon, delta, seed,t);
...
function plot_opinions(opinion_history)
    ...
    ylabel('Opinion value $x_i(t)$', 'Interpreter','latex');
...
T = 20; % Time steps
epsilon = 0.5; % The bounded confidence
delta= 0.1; % Minimum probability in random interaction
...
[op_history, W_series] = simulate(X0', C, T, epsilon, delta, seed);
figure(1);
...
figure(2);
plot_belief_network_evolution_dynamicW(X_series, W_series, coords, timepoints, {'a','b','c'}, opts);
drawnow;

[mu, varval] = calculate_plot_mean_variance(op_history, 21);
disp(mu);
disp("Variance");
disp(varval);